...
export function activate(context: vscode.ExtensionContext) {
  vscode.window.showInformationMessage('是否要打开OpenLinker的小站？', '是', '否', '不再提示').then((result) => {
    if (result === '是') {
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (showTip) {
    const result = await vscode.window.showInformationMessage('是否要打开OpenLinker的小站？', '是', '否', '不再提示')
    if (result === '是') {
      const commandLine = os.platform() === 'win32' ? `start https://kuizuo.cn` : `open https://kuizuo.cn`
      exec(commandLine)
    } else if (result === '不再提示') {
